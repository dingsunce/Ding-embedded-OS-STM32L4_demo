...
// You must call Rtc_GetDate() after Rtc_GetTime() to unlock the values in the higher - order
// calendar shadow registers to ensure consistency between the time and date values. Reading RTC
// current time locks the values in calendar shadow registers until Current date is read to ensure
...
u8 Rtc_GetYear(void)
{
  return __LL_RTC_CONVERT_BCD2BIN(LL_RTC_DATE_GetYear(RTC));
}
...
{
  return __LL_RTC_CONVERT_BCD2BIN(LL_RTC_DATE_GetMonth(RTC));
}
...
u8 Rtc_GetDay(void)
{
  return __LL_RTC_CONVERT_BCD2BIN(LL_RTC_DATE_GetDay(RTC));
}
...
u8 Rtc_GetWeek(void)
{
  return __LL_RTC_CONVERT_BCD2BIN(LL_RTC_DATE_GetWeekDay(RTC));
}
...
u8 Rtc_GetHour(void)
{
  return __LL_RTC_CONVERT_BCD2BIN(LL_RTC_TIME_GetHour(RTC));
}
...
{
  return __LL_RTC_CONVERT_BCD2BIN(LL_RTC_TIME_GetMinute(RTC));
}
...
{
  return __LL_RTC_CONVERT_BCD2BIN(LL_RTC_TIME_GetSecond(RTC));
}

#define PREDIV_S 255
// Second fraction = (PREDIV_S - SS) / (PREDIV_S + 1)
u16 Rtc_GetSubSecond(void)
{
  return (u16)((float)(PREDIV_S - LL_RTC_TIME_GetSubSecond(RTC)) / (float)(PREDIV_S + 1) * 1000.0);
}
...
static const u32 BackupRegisterList[] = {LL_RTC_BKP_DR0, LL_RTC_BKP_DR1, LL_RTC_BKP_DR2,
